...
NSString *const kBeaconIdentifier = @"com.anthonymichaelperry.beacon";
...
- (void)startAdvertisingUUID:(NSUUID *)uuid major:(CLBeaconMajorValue)major minor:(CLBeaconMinorValue)minor
{
    ...
    NSDictionary *peripheralData = [region peripheralDataWithMeasuredPower:@-34];
...
- (BOOL)bluetoothStateValid:(NSError **)error {
    ...
    switch (_peripheralManager.state) {
        case CBPeripheralManagerStatePoweredOff:
            if (error != NULL) {
                *error = [NSError errorWithDomain:@"com.anthonymichaelperry.bluetoothstate"
